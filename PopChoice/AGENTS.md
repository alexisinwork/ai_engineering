...
Group film recommender -- everyone answers four questions and rank fusion picks
one film. Lives directly in `ai_engineering`, not in its own repo.

Shares a Supabase project with `EmbeddingsAndVectorDB`, adding its own table
and match function rather than a second database.
...
npm install
cp .env.example .env
npm run ingest     # chains into eval via postingest
npm start
```
...
| `npm run ingest` | embed and store; chains into `eval` |
| `npm run eval` | ranking assertions |

## Conventions

- **This app always recommends; it is built around ranking, not refusal.** That
  is the deliberate contrast with `EmbeddingsAndVectorDB`, which must be able to
  say no and is therefore built around a calibrated threshold. Read the pair
  together before changing either -- same vector store, two different questions,
  two different shapes of eval.
- **It shares a Supabase project.** A migration here must not disturb the
  `podcasts` or `movies` tables, their indexes, or their match functions.
- Rank fusion combines per-person rankings. Do not collapse it into a single
  averaged embedding -- that loses the disagreement the fusion exists to
  resolve.
...
`.env` is gitignored. Never commit it, never print a key.

Vite inlines every `VITE_`-prefixed variable into the browser bundle, where
anyone can read it. A secret must never carry that prefix. `SUPABASE_SERVICE_ROLE_KEY`
in particular is RLS-bypassing and is Node-only.
